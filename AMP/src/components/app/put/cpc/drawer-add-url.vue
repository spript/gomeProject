					...
					<table class="table">
						<tbody>
							...
							<tr :class="{'actived': item.checked}" v-for="(item, index) in urlData.list">
								<td><span><el-radio v-model="selectPageId" :label="item.pageId" class="no_lable"></el-radio></span></td>
								<td><span>{{item.pageId}}</span></td>
	...
	methods: {
		...
		save() {
			let selectItem = this.urlData.list.find((item) => {
				return item.pageId === this.selectPageId;
			});
