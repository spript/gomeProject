<template>
 <div>
    <div class="account-set">
      ...
      <div class="set-fn">
        <div class="fn-plan">
          <div class="ele-input ele-input-01" style="width: 360px;">
            <input v-model="so.skuId" placeholder="输入商品SKU ID">
          </div>
        </div>
    ...
    <div class="el-dialog__wrapper" style="z-index: 1400;" v-if="dialogVisible">
    	<div class="el-dialog el-dialog--tiny" style="top: 15%;">
	    	...
	    	<div class="el-dialog__body"><span>确定要删除吗？</span> </div>
...
export default {
	...
	methods: {
		...
		addProductBySkuId() {
			...
			// 非空校验
			if (skuId === '') {
				this.$message({message: '请输入商品SKU ID', type: 'error'});
            	return;
            }
			...
			if (isExist) {
				this.$message({message: '此商品SKU ID列表中已存在', type: 'error'});
				return;
			}
			...
			.then((res) => {
				...
				.then((res) => {
					...
					if (!res.data.data.skuId) {
						this.$message({message: '查找不到与SKU ID相关的商品', type: 'error'});
						return;
					}
		...
		saveMaterial() {
			...
			console.log(params);
			console.log(this.drawerCtrlAction);
