<template>
	<div class="content-menu">
		<ul>
			<li v-for="(item, index) in routeConfig" @click="emitRoute(index)" :class="{'actived': item.checked}" v-privilege="item.privilege">
				...
				<div class="menu-second" v-bind:class="{'menu-second-new':item.msg == '用户协议'}">
					<a v-for="(subItem, subIndex) in item.sub" :class="{'actived': checkIsActive(subItem.link.replace('/#', ''))}" :href="sliceUrl(subItem.link)" title="" v-privilege="subItem.privilege">{{subItem.msg}}</a>
				</div>
			</li>
		</ul>
	</div>
