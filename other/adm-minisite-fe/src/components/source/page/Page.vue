<template>
	<div class="pageMain" style="height: 100%;overflow: auto">
		...
		<div class="item-list page-topic page-banner" v-for="(item,index) in topicArr"
			...
			<a :href="isInGome ? 'javascript:void(0)' :item.clickUrl" target="_blank">
				<img v-lazy="item.resource_url" alt="">
			</a>
		</div>
		<!--<div class="page-topic page-banner" v-if="topicArr.length > 0">
            <h3>精选活动</h3>
            <div class="item-list" v-for="(item,index) in topicArr"  :data-url="item.impressionUrl"
                 data-visible="visible">
                <a :href="isInGome ? 'javascript:void(0)' :item.clickUrl" target="_blank">
                    <img v-lazy="getThumbnailsUrl(item.resourceUrl, 260, 260)" alt="">
                </a>
            </div>
        </div>-->
		<!--猜你喜欢-->
		...
		<div :data-id="item.adId" class="item-list page-webpage page-banner" v-for="(item,index) in webpageArr"
			...
			<a :href="isInGome ? 'javascript:void(0)' : item.click_url">
				...
				<p class="webpage-desc" v-show="item.promotion_description.length > 0">{{ item.promotion_description }}</p>
			</a>
		</div>
		<!--	<div class="page-webpage page-banner" v-if="webpageArr.length > 0">
            <h3>有腔调</h3>
            <div class="item-list" v-for="(item,index) in webpageArr"
                 :data-url="item.impression_url" data-visible="visible"
                 @click="light(item.click_url, item.landing_url)">
                <a :href="isInGome ? 'javascript:void(0)' : item.click_url">
                    <img v-lazy="item.promotion_images[0]">
		...
		<!--	<div class="page-good page-banner" v-if="goodArr.length > 0">
            <h3>好东西</h3>
            <div class="item-list" v-for="(item,index) in goodArr"
                 :data-url="item.impression_url" data-visible="visible"
                 @click="light(item.click_url, item.landing_url)">
                <a :href="isInGome ? 'javascript:void(0)' : item.click_url">
                    <img v-lazy="item.promotion_images[0]">
...
<script>
	...
	import {getUrlInfoWithDeviceId, setStyles, getThumbnails, getCookie} from 'utils/common';
	...
	export default {
			...
			return {
				swiperOption: {
					...
					onSlideNextStart: (swiper) => {
						if (this.scroll <= 130) {
							this.slidesPression(swiper.activeIndex);
						}
					}
				},
				swiperOptionList: {
					...
					onTouchStart: (swiper) => {
					    if(swiper.activeIndex == this.likeArr.length - 4) return false;
						this.likePression(swiper.activeIndex, swiper.activeIndex + 4);
					},
		...
		methods: {
			...
			initLike() {
				let vm = this;
			    let url = $CONFIG['domain'] + '?slotId=' + this.likeSlotId + '&requestType=3&width_height=210-210&area=' + vm.areaId;
//				let url = 'http://flight-pre.gomeplus.com/flight?p=10136&catid=cat10000178&width_height=210-210&_=1493568000000&requestType=3&logLevel=0&area=11010200';
				jsonp(url, null, function(err, data) {
					for (let i = 0; i < data.length; i++) {
						let item = {};
			...
			initBothWebpageAndGood(sloatId, arr, count) {
				let vm = this;
				let url = $CONFIG['domain'] + '?slotId=' + sloatId + '&requestType=3&count=' + count;
				jsonp(url, null, function(err, data) {
			...
			//猜你喜欢轮播图曝光
			likePression(startIndex, endIndex) {
			    let vm = this;
				if(vm.likeArr.length == 0) return false;
