<template>
	<div>
		...
		<div class="account-set account-set-new mrt-0 pt-20 mb-25">
			...
			<div class="set-fn">
				<el-form :model="formData" :rules="rules" ref="formData">
					<div class="fn-plan" style="padding-top:20px;">
						...
						<span class="plan-title" style="margin-left:20px;">账户类型：</span>
						<el-select v-model="formData.accountType" placeholder="请选择" style="display:inline-block;width:160px;">
					...
					<div class="fn-plan" style="margin-top:20px">
						<span class="plan-title">消费类型：</span>
						<el-select v-model="formData.amountType" placeholder="请选择" style="display:inline-block;width:160px;">
							<el-option label="全部" :value="0"></el-option>
							<el-option label="广告账户" :value="1"></el-option>
							<el-option label="返利账户" :value="2"></el-option>
						</el-select>
					</div>
				</el-form>
		...
		<div class="account-set account-set-new mrt-0">
			<div class="set-tab">
				<router-link :to="{name: 'cosume-summary'}" active-class="active" tag="span" v-privilege="'report:record:cosume:summary'">汇总</router-link>
				<router-link :to="{name: 'cosume-detail'}" active-class="active" tag="span" v-privilege="'report:record:cosume:detail'">明细</router-link>
				...
				<div style="text-align:right;">
					<!--<a href="javascript:;" class='icon icon-manifest dms-a-self'><em class="dms-a-com">自定义指标</em></a>-->
					<a v-popover:popover2 href="javascript:;"  class="icon icon-report dms-a-report" v-privilege="'report:record:cosume:detail:export'"><em class="dms-a-com">导出</em></a>
				</div>
			</div>
				...
				<table class="table">
					...
					<tbody>
						<tr v-for="item in list">
							...
							<td v-show="checkedHeaders[1].checked" class="mw120"><span>{{item.advertiserId}}</span></td>
							<td v-show="checkedHeaders[2].checked" class="mw120"><span>{{item.advertiserName}}</span></td>
							<td v-show="checkedHeaders[3].checked" class="mw120"><span>{{item.accountType}}</span></td>
							<td v-show="checkedHeaders[4].checked" class="mw120"><span>{{item.category}}</span></td>
							<td v-show="checkedHeaders[5].checked" class="mw120"><span>{{item.amountType}}</span></td>
							<td v-show="checkedHeaders[6].checked" class="mw120"><span>{{item.consumeAmount}}</span></td>
...
const ADREPORTHEADERS = [{
		title: '效果指标',
		fields: [
			{msg: '消费类型', checked: true},
			{msg: '消费金额（元）', checked: true}
		]
	}
];
export default {
	name: "app-report-record-cosume-detail",
	data() {
		return {
			dateRange: [],
			formData: {
			...
			defaultHeaders: [
				...
				{msg: '账户类型', checked: true},
				{msg: '主营类目', checked: true},
				
			],
			breadcrumbConfig: {
				data: [{
					msg: '报表',
					link: '/#/app/report/record-cosume-summary'
				}, {
					msg: '充值消费记录',
					link: '/#/app/report/record-recharge-summary'
				}, {
					msg: '广告主消费',
					link: '/#/app/report/record-cosume-summary'
				}]
	...
	computed: {
		...
		getUrl() {
			return apiConfig[process.env.NODE_ENV] + '/api/consume/detail?';
		}
	},
	created() {
	...
	methods: {
		...
		getList() {
			http('api/report/anicer/consume/detail', {
				method: 'get',
				params: {
					startTime: this.formData.startTime,
					endTime: this.formData.endTime,
