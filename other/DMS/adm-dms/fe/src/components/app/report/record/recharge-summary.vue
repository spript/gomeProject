		...
		<div class="account-set account-set-new mrt-0">
			<div class="set-tab">
				...
				<div style="text-align:right;">
					...
					<a v-popover:popover2 href="javascript:;"  class="icon icon-report dms-a-report" v-privilege="'report:record:recharge:summary:export'"><em class="dms-a-com">导出</em></a>
				</div>
			</div>
				...
				<table class="table">
					...
					<tbody>
						<tr v-for="item in list">
							...
							<td v-show="checkedHeaders[2].checked" class="mw120"><span>{{item.accountType}}</span></td>
							<td v-show="checkedHeaders[3].checked" class="mw120"><span>{{item.rechargeType}}</span></td>
							<td v-show="checkedHeaders[4].checked" class="mw120"><span>{{item.total}}</span></td>
							<td v-show="checkedHeaders[5].checked" class="mw120"><span>{{item.historyTotal}}</span></td>
							<td v-show="checkedHeaders[6].checked" class="mw120"><span>{{item.current}}</span></td>
...
const ADREPORTHEADERS = [{
		title: '效果指标',
		fields: [
			{msg: '充值总金额（元）', checked: true},
			{msg: '历史充值总金额（元）', checked: true},
			{msg: '当前余额（元）', checked: true}
		]
	}
];
export default {
	name: "app-report-income-recharge-summary",
	data() {
		return {
			dateRange: [],
			formData: {
	...
	computed: {
		...
		getUrl() {
			return apiConfig[process.env.NODE_ENV] + '/api/recharge/summary?';
		}
	},
	created() {
	...
	methods: {
		...
		getList() {
			http('api/report/anicer/recharge/summary', {
				method: 'get',
				params: {
					startTime: this.formData.startTime,
					endTime: this.formData.endTime,
