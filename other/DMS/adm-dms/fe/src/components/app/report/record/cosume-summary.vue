		...
		<div class="account-set account-set-new mrt-0">
			<div class="set-tab">
				...
				<div style="text-align:right;">
					...
					<a v-popover:popover2 href="javascript:;"  class="icon icon-report dms-a-report" v-privilege="'report:record:cosume:summary:export'"><em class="dms-a-com">导出</em></a>
				</div>
			</div>
				...
				<table class="table">
					...
					<tbody>
						<tr v-for="item in list">
							...
							<td v-show="checkedHeaders[3].checked" class="mw120"><span>{{item.category}}</span></td>
							<td v-show="checkedHeaders[4].checked" class="mw120"><span>{{item.advertisementConsumeAmount}}</span></td>
							<td v-show="checkedHeaders[5].checked" class="mw120"><span>{{item.rebateConsumeAmount}}</span></td>
...
const ADREPORTHEADERS = [{
		title: '效果指标',
		fields: [
			{msg: '广告消费金额（元）', checked: true},
			{msg: '返利消费金额（元）', checked: true}
		]
	}
];
export default {
	name: "app-report-record-cosume-summary",
	data() {
		return {
			dateRange: [],
			formData: {
	...
	computed: {
		...
		getUrl() {
			return apiConfig[process.env.NODE_ENV] + '/api/consume/summary?';
		}
	},
	created() {
	...
	methods: {
		...
		getList() {
			http('api/report/anicer/consume/summary', {
				method: 'get',
				params: {
					startTime: this.formData.startTime,
					endTime: this.formData.endTime,
