			...
			<div class="set-fn">
				<el-form :model="formData" :rules="rules" ref="formData">
					<div class="fn-plan" style="padding-top:20px;">
						...
						<span class="plan-title" style="margin-left:20px">设备类型：</span>
					...
					<div class="fn-plan" style = "margin-top:20px">
						<span class="plan-title">计费类型：</span>
		...
		<div class="account-set account-set-new mrt-0">
			<div class="set-tab">
				...
				<div style="text-align:right;">
					...
					<a v-popover:popover2 href="javascript:;"  class="icon icon-report dms-a-report" v-privilege="'report:income:anice:export'"><em class="dms-a-com">导出</em></a>
				</div>
			</div>
				...
				<table class="table">
					...
					<tbody>
						<tr v-for="item in list">
							...
							<td v-show="checkedHeaders[1].checked" class="mw120"><span>{{item.mediaName}}</span></td>
							<td v-show="checkedHeaders[2].checked" class="mw120"><span>{{item.deviceType}}</span></td>
							<td v-show="checkedHeaders[3].checked" class="mw120"><span>{{item.sellType}}</span></td>
							<td v-show="checkedHeaders[4].checked" class="mw120"><span>{{item.totalFee}}</span></td>
							<td v-show="checkedHeaders[5].checked" class="mw120"><span>{{item.averageClickCost}}</span></td>
							<td v-show="checkedHeaders[6].checked" class="mw120"><span>{{item.averageImpCost}}</span></td>
						</tr>
					</tbody>
				</table>
			</div>
...
const ADREPORTHEADERS = [{
		...
		fields: [
			...
			{msg: '收入(元)', checked: true},
			{msg: '平均点击单价（元）', checked: true},
			{msg: '千次展现成本（元）', checked: true}
		]
	}
];
export default {
	name: "app-report-income-anice-income",
	data() {
		return {
			dateRange: [],
			formData: {
	...
	computed: {
		...
		getUrl() {
			return apiConfig[process.env.NODE_ENV] + '/api/anice/income?';
		}
	},
	created() {
