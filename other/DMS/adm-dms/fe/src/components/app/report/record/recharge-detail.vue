<template>
	<div>
			...
			<div class="set-fn pt-20">
				<el-form :model="formData" :rules="rules" ref="formData">
					<div class="fn-plan">
						...
						<el-select v-model="formData.accountType" placeholder="请选择" style="display:inline-block;width:120px;">
		...
		<div class="account-set account-set-new mrt-0">
			<div class="set-tab">
				<router-link :to="{name: 'recharge-summary'}" active-class="active" tag="span" v-privilege="'report:record:recharge:summary'">汇总</router-link>
				<router-link :to="{name: 'recharge-detail'}" active-class="active" tag="span" v-privilege="'report:record:recharge:detail'">明细</router-link>
				...
				<div style="text-align:right;">
					...
					<a v-popover:popover2 href="javascript:;"  class="icon icon-report dms-a-report" v-privilege="'report:record:recharge:detail:export'"><em class="dms-a-com">导出</em></a>
				</div>
			</div>
				...
				<table class="table">
					...
					<tbody>
						<tr v-for="item in list">
							...
							<td v-show="checkedHeaders[4].checked" class="mw120"><span>{{item.chargeType}}</span></td>
							<td v-show="checkedHeaders[5].checked" class="mw120"><span>{{item.chargeAmout}}</span></td>
							<td v-show="checkedHeaders[6].checked" class="mw120"><span>{{item.orderNo}}</span></td>
...
const ADREPORTHEADERS = [{
		title: '效果指标',
		fields: [
			{msg: '充值金额（元）', checked: true},
			{msg: '流水单号', checked: true}
		]
	}
];
export default {
	name: "app-report-record-recharge-detail",
	data() {
		return {
			dateRange: [],
			formData: {
	...
	computed: {
		...
		getUrl() {
			return apiConfig[process.env.NODE_ENV] + '/api/recharge/detail?';
		}
	},
	created() {
	...
	methods: {
		...
		getList() {
			http('api/report/anicer/recharge/detail', {
				method: 'get',
				params: {
					startTime: this.formData.startTime,
					endTime: this.formData.endTime,
