<template>
	<div>
		...
		<div class="account-set account-set-new mrt-0">
			<div class="set-tab">
				<router-link :to="{name: 'anice-summary'}" active-class="active" tag="span" v-privilege="'report:anicer:anice:summary'">汇总</router-link>
				<router-link :to="{name: 'anice-detial'}" active-class="active" tag="span" v-privilege="'report:anicer:anice:detail'">明细</router-link>
				...
				<div style="text-align:right;">
					...
					<a v-popover:popover2 href="javascript:;"  class="icon icon-report dms-a-report" v-privilege="'report:anicer:anice:summary:export'"><em class="dms-a-com">导出</em></a>
				</div>
			</div>
				...
				<table class="table">
					...
					<tbody>
						<tr v-for="item in list">
							<td v-show="checkedHeaders[0].checked" class="mw120"><span>{{item.adAdvertiserId}}</span></td>
							<td v-show="checkedHeaders[1].checked" class="mw120"><span>{{item.adAdvertiserName}}</span></td>
							<td v-show="checkedHeaders[2].checked" class="mw120"><span>{{item.advertiserType}}</span></td>
							<td v-show="checkedHeaders[3].checked" class="mw120"><span>{{item.displayCount}}</span></td>
							<td v-show="checkedHeaders[4].checked" class="mw120"><span>{{item.displayUv}}</span></td>
							<td v-show="checkedHeaders[5].checked" class="mw120"><span>{{item.clickCount}}</span></td>
							<td v-show="checkedHeaders[6].checked" class="mw120"><span>{{item.clickRate}}</span></td>
							<td v-show="checkedHeaders[7].checked" class="mw120"><span>{{item.avgClickCost}}</span></td>
							<td v-show="checkedHeaders[8].checked" class="mw120"><span>{{item.thousandDisplayCost}}</span></td>
							<td v-show="checkedHeaders[9].checked" class="mw120"><span>{{item.totalFee}}</span></td>
							<td v-show="checkedHeaders[10].checked" class="mw120"><span>{{item.roi}}</span></td>
							<td v-show="checkedHeaders[11].checked" class="mw120"><span>{{item.totalPrice}}</span></td>
							<td v-show="checkedHeaders[12].checked" class="mw120"><span>{{item.directProductLineNum}}</span></td>
							<td v-show="checkedHeaders[13].checked" class="mw120"><span>{{item.directProductLineAmount}}</span></td>
							<td v-show="checkedHeaders[14].checked" class="mw120"><span>{{item.indirectProductLineNum}}</span></td>
							<td v-show="checkedHeaders[15].checked" class="mw120"><span>{{item.indirectProductLineAmount}}</span></td>
							<td v-show="checkedHeaders[16].checked" class="mw120"><span>{{item.totalNum}}</span></td>
						</tr>
					</tbody>
				</table>
			</div>
...
export default {
	name: "app-report-anicer-anice-summary",
	data() {
		return {
			dateRange: [],
			formData: {
	...
	computed: {
		...
		getUrl() {
			return apiConfig[process.env.NODE_ENV] + '/api/anice/summary?';
		}
	},
	created() {
	...
	methods: {
		...
		getList() {
			http('api/report/anicer/anice/summary', {
				method: 'get',
				params: {
					startTime: this.formData.startTime,
					endTime: this.formData.endTime,
