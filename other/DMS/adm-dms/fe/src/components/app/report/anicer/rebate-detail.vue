		...
		<div class="account-set account-set-new mrt-0">
			<div class="set-tab">
				...
				<div style="text-align:right;">
					...
					<a v-popover:popover2 href="javascript:;"  class="icon icon-report dms-a-report" v-privilege="'report:anicer:rebate:detail:export'"><em class="dms-a-com">导出</em></a>
				</div>
			</div>
				...
				<table class="table">
					...
					<tbody>
						<tr v-for="item in list">
							...
							<td v-show="checkedHeaders[4].checked" class="mw120"><span>{{item.mediaName}}</span></td>
							<td v-show="checkedHeaders[5].checked" class="mw120"><span>{{item.campaginId}}</span></td>
							<td v-show="checkedHeaders[6].checked" class="mw120"><span>{{item.landPv}}</span></td>
							<td v-show="checkedHeaders[7].checked" class="mw120"><span>{{item.landUv}}</span></td>
							<td v-show="checkedHeaders[8].checked" class="mw120"><span>{{item.sharedPv}}</span></td>
							<td v-show="checkedHeaders[9].checked" class="mw120"><span>{{item.sharedUv}}</span></td>
							<td v-show="checkedHeaders[10].checked" class="mw120"><span>{{item.chargeShareNumber}}</span></td>
							<td v-show="checkedHeaders[11].checked" class="mw120"><span>{{item.avgClickCost}}</span></td>
...
export default {
	name: "app-report-anicer-rebate-detail",
	data() {
		return {
			dateRange: [],
			formData: {
	...
	computed: {
		...
		getUrl() {
			return apiConfig[process.env.NODE_ENV] + '/api/rebate/detail?';
		}
	},
	created() {
	...
	methods: {
		...
		getList() {
			http('api/report/anicer/rebate/detail', {
				method: 'get',
				params: {
					startTime: this.formData.startTime,
					endTime: this.formData.endTime,
