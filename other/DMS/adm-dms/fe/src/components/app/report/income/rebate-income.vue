		...
		<div class="account-set account-set-new mrt-0">
			<div class="set-tab">
				...
				<div style="text-align:right;">
					...
					<a v-popover:popover2 href="javascript:;"  class="icon icon-report dms-a-report" v-privilege="'report:income:rebate:export'"><em class="dms-a-com">导出</em></a>
				</div>
			</div>
				...
				<table class="table">
					...
					<tbody>
						<tr v-for="item in list">
							...
							<td v-show="checkedHeaders[3].checked" class="mw120"><span>{{item.chargeShareNumber}}</span></td>
							<td v-show="checkedHeaders[4].checked" class="mw120"><span>{{item.rebateCost}}</span></td>
							<td v-show="checkedHeaders[5].checked" class="mw120"><span>{{item.averageShareCost}}</span></td>
						</tr>
					</tbody>
				</table>
			</div>
...
const ADREPORTHEADERS = [{
		...
		fields: [
			...
			{msg: '返利花费（元）', checked: true},
			{msg: '平均分享单价（元）', checked: true}
		]
	}
];
export default {
	name: "app-report-income-rebate-income",
	data() {
		return {
			dateRange: [],
			formData: {
	...
	computed: {
		...
		getUrl() {
			return apiConfig[process.env.NODE_ENV] + '/api/rebate/income?';
		}
	},
	created() {
