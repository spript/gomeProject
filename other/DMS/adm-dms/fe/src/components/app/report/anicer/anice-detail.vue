				...
				<el-form :model="formData" :rules="rules" ref="formData">
					<div class="fn-plan">
						...
						<el-select v-model="formData.optionType" placeholder="请选择" style="display:inline-block;width:120px;margin-left:20px;">
							...
							<el-option label="投放计划id" :value="2"></el-option>
						</el-select>
		...
		<div class="account-set account-set-new mrt-0">
			<div class="set-tab">
				...
				<el-popover ref="popover2" placement="bottom" width="260" trigger="click">
					<template v-if="list.length > 0">
						...
						<el-select v-model="exportData.startPage">
							<el-option v-for="item in 100" :label="item" :value="item"></el-option>
						</el-select>
				...
				<div style="text-align:right;">
					...
					<a v-popover:popover2 href="javascript:;"  class="icon icon-report dms-a-report" v-privilege="'report:anicer:anice:detail:export'"><em class="dms-a-com">导出</em></a>
				</div>
			</div>
				...
				<table class="table">
					...
					<tbody>
						<tr v-for="item in list">
							...
							<td v-show="checkedHeaders[1].checked" class="mw120"><span>{{item.adAdvertiserId}}</span></td>
							<td v-show="checkedHeaders[2].checked" class="mw120"><span>{{item.adAdvertiserName}}</span></td>
							<td v-show="checkedHeaders[3].checked" class="mw120"><span>{{item.advertiserType}}</span></td>
							<td v-show="checkedHeaders[4].checked" class="mw120"><span>{{item.campaginId}}</span></td>
							<td v-show="checkedHeaders[5].checked" class="mw120"><span>{{item.deviceType}}</span></td>
							<td v-show="checkedHeaders[6].checked" class="mw120"><span>{{item.displayCount}}</span></td>
							<td v-show="checkedHeaders[7].checked" class="mw120"><span>{{item.displayUv}}</span></td>
							<td v-show="checkedHeaders[8].checked" class="mw120"><span>{{item.clickCount}}</span></td>
							<td v-show="checkedHeaders[9].checked" class="mw120"><span>{{item.clickRate}}</span></td>
							<td v-show="checkedHeaders[10].checked" class="mw120"><span>{{item.avgClickCost}}</span></td>
							<td v-show="checkedHeaders[11].checked" class="mw120"><span>{{item.thousandDisplayCost}}</span></td>
							<td v-show="checkedHeaders[12].checked" class="mw120"><span>{{item.totalFee}}</span></td>
							<td v-show="checkedHeaders[13].checked" class="mw120"><span>{{item.roi}}</span></td>
							<td v-show="checkedHeaders[14].checked" class="mw120"><span>{{item.totalPrice}}</span></td>
							...
							<td v-show="checkedHeaders[19].checked" class="mw120"><span>{{item.totalNum}}</span></td>
						</tr>
					</tbody>
				</table>
			</div>
...
export default {
	name: "app-report-anicer-anice-detail",
	data() {
		return {
			dateRange: [],
			formData: {
			...
			rules: {
				...
				optionTypeText: [{
					validator: (rule, value, callback) => {
						if(value && (this.formData.optionType == 0 ||this.formData.optionType == 2)) {
							let regex = /^\d+($|(\,\d+){0,9}$)/;
							if (!regex.test(value)) {
	...
	computed: {
		...
		getUrl() {
			return apiConfig[process.env.NODE_ENV] + '/api/anice/detail?';
		}
	},
	created() {
	...
	methods: {
		...
		getList() {
			http('api/report/anicer/anice/detail', {
				method: 'get',
				params: {
					startTime: this.formData.startTime,
					endTime: this.formData.endTime,
